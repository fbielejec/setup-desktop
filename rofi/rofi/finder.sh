...
then
  ...
  then
    ...
    done <<< $(find /home/filip -iname *"${QUERY#\?}"* 2>&1 | grep -v 'Permission denied\|Input/output error')      
  else
    # find / -iname *"${QUERY#!}"* 2>&1 | grep -v 'Permission denied\|Input/output error'
    find /home/filip -iname *"${QUERY#!}"* 2>&1 | grep -v 'Permission denied\|Input/output error'    
  fi
else
  echo "!!-- Type your search query to find files"
